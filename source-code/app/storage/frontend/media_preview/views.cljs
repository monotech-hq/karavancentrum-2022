...
(defn- media-preview-data
  ...
  (let [media-alias       @(r/subscribe [:x.db/get-item [:storage :media-preview/downloaded-items id :alias]])
       ...
       [common/data-table {:disabled? disabled?
                           :rows [[{:content :filename} {:content media-alias :color :muted :placeholder "-"}]
                                  [{:content :filesize} {:content media-size  :color :muted :placeholder "-"}]
                                  [{:content :uploaded} {:content timestamp   :color :muted :placeholder "-"}]]}]))
